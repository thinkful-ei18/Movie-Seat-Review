...
function getResponseFromParameters(parameters, serverRes) {
  ...
  var apiKey = '2phuffebn3xk7y98kduaakt8';
  var sharedSecret = 'gshqEf2kuP';
...
var startupMsg = '<=-- Load this url to see Fandango response: http://{0}:{1}    (<ctrl-c> to quit) --=>'.format(
  localhost,
  port
);
...
var http = require('http');
http
  .createServer(function(req, serverRes) {
    serverRes.writeHead(200, { 'Content-Type': 'text/xml' });

    var zipCode = '90064';
    var parameters = 'op=theatersbypostalcodesearch&postalcode={0}'.format(
      zipCode
    );
